...
const roleTypeMap = {
  1: VHDocSDK.RoleType.HOST, // 1     主持人[1]
  2: VHDocSDK.RoleType.SPECTATOR, // 3 观众[2]
  3: VHDocSDK.RoleType.ASSISTANT, // 4 助理[3]
  4: VHDocSDK.RoleType.GUEST // 2 嘉宾[4]
};
export default {
  props: {
    rebroadcast: {},
  ...
  watch: {
    docPermissionId (newval) {
      ...
      if (this.hasDocPermission) {
        ...
        this.isSetRole = 1;
        this.docSDK && this.docSDK.setRole(roleTypeMap[1]).then(res => {
        ...
        if (this.docInfo.docShowType == 'document') {
          this.docID && EventBus.$emit('remote_doc_select', { id: this.docID });
        } else if (this.docInfo.docShowType == 'board') {
          this.boardID &&
            EventBus.$emit('remote_doc_select', { id: this.boardID });
        }
      } else {
        if (this.roleName == 4 || this.roleName == 1) {
          this.isSetRole = 4;
          this.docSDK && this.docSDK.setRole(roleTypeMap[4]).then(res => {
            console.log('文档---切换权限成功');
            this.isSetRole = -1;
          }).catch(() => {
          // 实例未全部加载完毕，切换失败
  ...
  methods: {
    ...
    toggleWatchContainer (flag) {
      ...
      EventBus.$emit('component_doc_watch_containner', flag);
    },
    ...
    _listenEvents () {
      ...
      // 文档列表组件触发演示文档事件
      EventBus.$on('demonstration', data => {
      ...
      EventBus.$on('live_broadcast_start', msg => {
        this.docSDK && this.docSDK.setRole(roleTypeMap[2]);
        this.docSDK &&
        ...
        this.rebroadcastStartTimer = setTimeout(() => {
          this.docSDK && this.docSDK.setPlayMode(VHDocSDK.PlayMode.FLV);
          this.loadRemote();
      ...
      EventBus.$on('live_broadcast_stop', msg => {
        ...
        this.rebroadcastStopTimer = setTimeout(() => {
          EventBus.$emit('documenet_load_complete', []);
          this.boardID = '';
          this.docID = '';
          this.cids = [];
          this.currentCid = '';
          ...
          this.docSDK && this.docSDK.setRole(roleTypeMap[1]);
          this.docSDK && this.docSDK.setPlayMode(VHDocSDK.PlayMode.INTERACT);
    ...
    _initDocSDK () {
      ...
      let opt = {
        ...
        role: roleTypeMap[this.disableAssistant ? 2 : this.roleName], // 角色 必须, 助理无文档权限时设置观众权限
      ...
      let success = () => {
        this.docLoadComplete = false;
        if (this.hasDocPermission) {
          this.docSDK.setRole(roleTypeMap[1]);
        } else {
          this.docSDK.setRole(
            roleTypeMap[this.disableAssistant ? 2 : this.roleName]
          );
        }
        // 刷新时，如果在直播中会加载远端文档信息
        if (this.roleName == 1) {
          if (this.rebroadcastChannelId) {
            ...
            this.docInfo.showInWatch = false;
            EventBus.$emit('docInfo', this.docInfo);
            this.docSDK.setRole(roleTypeMap[2]);
            this.docSDK.setPlayMode(VHDocSDK.PlayMode.FLV);
            this.hasDocPermission = false;
          }
          this.loadRemote();
        } else {
        ...
        EventBus.$emit('docSDK_ready', this.docSDK);
      ...
      this.docSDK.on(VHDocSDK.Event.DOCUMENT_LOAD_COMPLETE, data => {
      ...
      this.docSDK.on(VHDocSDK.Event.PAGE_CHANGE, data => {
        console.log('===================文档翻页====================', data);
        EventBus.$emit('PAGE_CHANGE', data.info.slideIndex);
        this.pageInfo = {
          total: data.info.slidesTotal,
          pageIndex: Number(data.info.slideIndex) + 1
        };
      ...
      this.docSDK.on(VHDocSDK.Event.CREATE_CONTAINER, data => {
        if (
      ...
      this.docSDK.on(VHDocSDK.Event.ALL_COMPLETE, () => {
        console.log('=============所有文档加载完成==============');
        this.docLoadComplete = true;
        if (this.isSetRole > -1) {
          this.docSDK.setRole(roleTypeMap[this.isSetRole]).then(res => {
            this.isSetRole = -1;
          }).catch(() => {
            // 实例未全部加载完毕，切换失败
            console.log('=============文档没有在完成===========');
    ...
    activeContainer (cid = '', isEvent = true) {
      ...
      this.currentCid = cid;
      isEvent && EventBus.$emit('remote_doc_select', { id: cid });
...
.doc-wrapper {
  ...
  .doc-empty {
    width: 100%;
    height: 100%;
    background: url('../../assets/images/publish/doc-default.png') no-repeat;
