...
export default {
  ...
  methods: {
    initPaly () {
      ...
      this.initSDK().then(() => {
        ...
        if (this.isLive) {
          ...
          this.$PLAYER.on(VhallPlayer.PLAY, () => {
            // 监听播放状态
            this.statePaly = true;
            this.setChange();
          ...
          this.$PLAYER.on(VhallPlayer.PAUSE, () => {
            // 监听暂停状态
            console.log('暂停');
            this.statePaly = false;
          });
          // 视频清晰度发生改变----卡顿切换清晰度时触发
          this.$PLAYER.on(VhallPlayer.DEFINITION_CHANGE, () => {
            this.loading = true;
          });
          ...
          this.$PLAYER.on(VhallPlayer.ERROR, (e) => {
          ...
          this.$PLAYER.on(VhallPlayer.ENDED, () => {
            // 监听暂停状态
            console.log('播放完毕');
            this.BackEnd = true;
    ...
    initSlider () {
      this.$PLAYER.on(VhallPlayer.TIMEUPDATE, () => {
