      ...
      <div class="vhsaas-reg-login__main">
        ...
        <div v-if="tag != 3" class="vhsaas-login-box">
          <p class="vhsaas-box-title">
            <span :class="tag == 1 ? 'active' : ''" @click="changeTagHandler(1)">账号登录</span>
            <em>|</em>
            <span :class="tag == 2 ? 'active' : ''" @click="changeTagHandler(2)">快捷登录</span>
          </p>
          <!-- 账号登录 -->
          <el-form v-if="tag == 1" key="loginOneForm" ref="ruleForm" :model="ruleForm" :rules="ruleFormRules"  label-width="0">
            ...
            <el-form-item class="vhsaas-box__pwd" prop="password">
              <el-input v-if="loginPwdShow" v-model.trim="ruleForm.password" clearable :maxlength="20" placeholder="请输入登录密码" type="password" @focus="loginErrMsg = ''">
                <i slot="suffix" style="font-size:14px;margin-right:5px;cursor: pointer;" class="iconfont iconyincang_icon" @click="loginPwdShow = false"></i>
              </el-input>
              <el-input v-else v-model.trim="ruleForm.password" clearable :maxlength="20" placeholder="请输入登录密码" @focus="loginErrMsg = ''">
                <i slot="suffix" style="font-size:12px;margin-right:5px;cursor: pointer;" class="iconfont iconxianshi_icon" @click="loginPwdShow = true"></i>
              </el-input>
            </el-form-item>
            <!-- 其它 -->
            <el-form-item>
              <div :class="['vhsaas-box__link', {'vhsaas-box__link__error' : mailError, 'vhsaas-box__msg__error': photoCpathaShow}]">
                ...
                <span class="vhsaas-box__auto">自动登录</span>
        ...
        <div v-else class="vhsaas-reg-box">
          <p class="vhsaas-box-title reg">
            <span class="active no-border">欢迎注册微吼直播</span>
          </p>
          <el-form  key="regForm" ref="regForm" :model="regForm" :rules="regFormRules">
            ...
            <!-- 短信密码 -->
            <el-form-item prop="code" :class="['vhsaas-wrap-code', {'vhsaas-box__msg__error' : !smsRegErrMsg}]">
              <el-input v-model.trim="regForm.code"  clearable type="captcha" :maxlength="6" placeholder="动态验证码"  @focus="smsRegErrMsg = ''"></el-input>
            ...
            <el-form-item :class="['vhsaas-box__pwd', 'vhsaas-box__msg__error']"  prop="password">
              <el-input v-if="regPwdShow" v-model.trim="regForm.password" clearable :maxlength="20" placeholder="请设置登录密码" type="password" @focus="regErrMsg = ''">
                <i slot="suffix" style="font-size:14px;margin-right:5px;cursor: pointer;" class="iconfont iconyincang_icon" @click="regPwdShow = false"></i>
              </el-input>
              <el-input v-else v-model.trim="regForm.password" clearable :maxlength="20" placeholder="请设置登录密码" @focus="regErrMsg = ''">
                <i slot="suffix" style="font-size:12px;margin-right:5px;cursor: pointer;" class="iconfont iconxianshi_icon" @click="regPwdShow = true"></i>
              </el-input>
              <!-- 去登录 -->
