...
export default {
  ...
  created() {
    // 创建
    // const that = this
    // this.$VhallEventBus.$on(this.$VhallEventType.Nav.NAV_LOGIN_SHOW, isShow => {
    //   if (isShow) {
    //     that.dialogVisible = true
    //   } else {
    //     that.closeDialogHandler()
    //   }
    // })
  },
  mounted() {
  ...
  methods: {
    ...
    // 校验登录次数
    checkLoginAccount() {
      this.$fetch('loginCheck', {account: this.ruleForm.username}).then(res => {
    ...
    fetchData() {
      this.$fetch('getInfo', {
        ...
        // 登录成功，事件派发
        // this.$VhallEventBus.$emit(this.$VhallEventType.Nav.NAV_LOGIN_SUCCESS)
        // 登录成功，关闭弹出框
    ...
    registerHandler() {
      ...
      this.regForm.source = this.$route.query.source || 1
        ...
        if (valid) {
          this.$fetch('register', {
            ...
            if (res && res.code === 200) {
              ...
              this.changeTagHandler(1) // 切换至登录
            } else {
              this.regPhoneErr = res.msg || '注册失败'
            }
          }).catch(e => {
            console.log(e)
            this.regPhoneErr = '注册失败'
          })
        }
      })
    },
    ...
    handleBindPath() {
      ...
      // 第三方登录地址
      this.wxPath = `${hostPath}/commons/auth/weixin?source=pc&jump_url=` + encodeURIComponent(jumpUrlPath)
      this.qqPath = `${hostPath}/commons/auth/qq?source=pc&jump_url=` + encodeURIComponent(jumpUrlPath)
    }
  },
  destroyed() {
    // 死亡
