      ...
      <el-col :span="6" v-if="titleType">
       ...
       <el-col :span="6" v-if="!titleType">
        <div class="grid-content">
          <span>活动总数</span>
          <el-tooltip effect="dark" placement="right" v-tooltipMove>
            <div slot="content">筛选条件内创建的活动总数，包含直播、点播<br />删除活动或者删除子账号不影响已统计的历史数据</div>
            <i class="iconfont-v3 saasicon_help_m"></i>
          </el-tooltip>
          <h3 class="custom-font-barlow">
            <count-to :startVal="0"
        ...
        <div class="grid-content">
          ...
          <el-tooltip effect="dark" placement="right" v-tooltipMove>
            ...
            <div slot="content" v-else>
              {{ titleType==4 ? '点播' : '直播'}}创建至今，进入观看页面{{ titleType == 4 ? '' : '（直播+回放）'}}的观看人数，以播放器sdk上报为准，<br />用户真实观看了视频。筛选条件内同一个用户多次观看会进行去重
            </div>
            <i class="iconfont-v3 saasicon_help_m"></i>
          </el-tooltip>
          <h3 class="custom-font-barlow">
