<template>
  <el-upload class="avatar-uploader" v-bind="$props" :headers="headersVo" :action="action" :data="saveData"
    name="resfile" accept="image/jpg,image/jpeg,image/png,image/gif,image/bmp" :on-success='handleuploadSuccess'>
    <div class="box">
      <div v-if="value || domain_url && domain_url.indexOf('aliqr.e.vhall.com/qr.png') == -1">
        <div :style="`width:${widthImg}px;height:${heightImg}px`" class="img-logo"><img :src="domain_url || domainUrl"
            class="avatar" alt="" @click.stop.prevent="!isFullCover&&fullCover()" /></div>
