          ...
          <vue-scroll :ops="ops">
            <ul class="chapterList">
              <li v-for="(item, index) in tableData" @click="chapterHandler(index)" :key="index">
                <span class="title">
                  <span class="titleIndex">{{ item.index }}.</span>
                  {{ item.title }}
                </span>
...
<script>
  ...
  export default {
    ...
    methods: {
      calcHeight() {
        document.querySelector('.chaptersBox').style.height =
          document.querySelector('.playerBoxContainer').offsetHeight -
          document.querySelector('.playerBox').offsetHeight +
          'px';
      },
      videoMouseOver() {
      ...
      chapterHandler(index) {
        ...
        this.$refs.player.$PLAYER.setCurrentTime(this.tableData[index].createTime, e =>
          console.log(e)
        );
      },
      getPlayBackInfo() {
