  ...
  <div class="listBox">
    ...
    <template v-if="no_show === false">
      <div v-if="!isDemand" class="operaBlock">
        ...
        <VhallInput
          ...
          v-model="keyWords"
        >
          <i class="el-icon-search el-input__icon" slot="prefix" @click="getSearchList"></i>
        </VhallInput>
      </div>
      <div :class="['tableBox', isDemand === true ? '' : 'recordTableBox']" v-loading="loading">
        ...
        >
          ...
          ></el-table-column>
          <el-table-column :width="isDemand === true ? 375 : 365" label="内容标题">
            <template slot-scope="scope">
              ...
              <div class="content">
                <div class="info" @click="preview(scope.row)" style="cursor: pointer">
                  ...
                  <p>
                    第{{ scope.row.group_switch_num }}次分组：{{ scope.row.group_name }}
                    <span
                      v-if="scope.row.doc_status && WEBINAR_PES['ui.record_chapter']"
                      class="tag"
                    >
                      章节
                    </span>
          ...
          <el-table-column label="状态" :width="isBidScreen ? '' : 91" show-overflow-tooltip>
            <template slot-scope="scope">
          ...
          <el-table-column label="时长" :width="isBidScreen ? '' : 91" show-overflow-tooltip>
          ...
          <el-table-column :width="isBidScreen ? 190 : 176" label="操作" align="left">
            <template slot-scope="scope">
              {{ scope.row.date }}
              <el-button type="text" @click="editDialog(scope.row)">编辑</el-button>
              <el-button v-if="scope.row.source != 2" type="text" @click="downPlayBack(scope.row)">
                下载
              </el-button>
              <el-button v-if="is_rehearsal" type="text" @click="deletePlayBack(scope.row.id, 2)">
                删除
              </el-button>
              <el-button
                v-if="WEBINAR_PES['ui.record_chapter'] && !is_rehearsal"
                type="text"
              ...
              <el-dropdown v-if="!isDemand && !is_rehearsal" @command="handleCommand">
                ...
                <el-dropdown-menu style="width: 160px" slot="dropdown">
                  <el-dropdown-item
                    v-if="WEBINAR_PES['reset_record'] && !scope.row.layout"
                  ...
                  <el-dropdown-item :command="{ command: 'delete', data: scope.row }">
                    删除
                  </el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="confirmEdit" :disabled="editLoading" round size="medium">
          确定
        </el-button>
        <el-button @click="editDialogVisible = false" :disabled="editLoading" round size="medium">
          取消
        </el-button>
      </span>
    </el-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        this.loading = true;
        // 获取小组回放列表
        this.$fetch('getGroupRecordList', param)
          .then(res => {
