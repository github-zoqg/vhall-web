  ...
  <div class="listBox">
    ...
    <template v-if="no_show === false">
      <div v-if="!isDemand" class="operaBlock">
        ...
        <el-button size="medium" class="transparent-btn" round @click="settingHandler">
          回放设置
        </el-button>
        <el-button
          size="medium"
      ...
      <div :class="['tableBox', isDemand === true ? '' : 'recordTableBox']" v-loading="loading">
        ...
        >
          ...
          <el-table-column :width="isBidScreen ? 190 : 176" label="操作" fixed="right" align="left">
            <template slot-scope="scope">
              {{ scope.row.date }}
              ...
              <el-button
                v-if="
                  !(
                    liveDetailInfo.webinar_state == 4 ||
                    [5, '5'].includes(liveDetailInfo.webinar_type)
                  )
                "
                :disabled="scope.row.source == 2"
                type="text"
                @click="downPlayBack(scope.row)"
              >
                下载
              </el-button>
              <el-button
