  ...
  <div :class="['chooseWay', { 'no-login': executeType !== 'ctrl' }]">
    ...
    <div class="choose__way__main">
      <div class="choose__way__ctx">
        ...
        <div class="choose-btn">
          <el-button type="primary" round @click="goLive(2)" class="length152" v-preventReClick>
            {{ arr[1] == 1 ? '发起直播' : '进入直播' }}
          </el-button>
        ...
        >
          客户端启动遇到问题？您可以尝试：
          <a target="_blank" :href="downloadUrl">下载客户端</a>
          联系客服：400-888-9970
        </div>
      </div>
    </div>
    ...
    >
      <div class="director-dialog director-types-dialog">
        ...
        <el-button type="primary" round @click="openLive" class="btn" v-preventReClick>
          确认
        </el-button>
      </div>
    </el-dialog>
    ...
    methods: {
      ...
      getRoleUrl() {
        ...
        let params = {
          ...
          type: this.arr[1],
          live_token:
            Number(this.arr[1]) !== 1 ? sessionOrLocal.get('liveToken', 'localStorage') : ''
        }; // 若非主持人登录，需传递用户token
        if (
        ...
        this.$fetch('getJoinUrl', this.$params(params), {
          platform:
            this.executeType === 'ctrl' ? sessionOrLocal.get('platform', 'localStorage') || 17 : 7,
          'gray-id': this.gray_id
        })
          .then(res => {
            if (res && res.code === 200) {
    ...
    mounted() {
      ...
      if (checkInfo) {
        ...
        this.hasDelayPermission =
          checkInfo['no.delay.webinar'] && checkInfo['no.delay.webinar'] == 1 ? true : false;
      }
