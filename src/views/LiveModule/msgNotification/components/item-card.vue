    ...
    <div :class="`item-card-center ${info && info.config_type == 1 ? 'css_flex' : ''}`">
      ...
      <template v-if="!(info.content instanceof Array)">
        <div class="item-card-center__ctx">
          {{ info.content_str_min }}
          <span @click="openDialog('link')" class="item-card-center__link">
            {{ hideString(info.short_url, 20) }}
          </span>
        </div>
      </template>
      <!-- 情况而： 微信活动内容区域 -->
      <template v-else>
        ...
        <div>
          <p
            v-for="(subItem, subIndex) in info.content"
            :key="`t-item-${subIndex}`"
            class="item-card_ct_item"
          >
    ...
    <div class="item-card-bottom">
      <div class="item-card-bottom__default">
        ...
        <p v-if="info && info.config_type == 2" class="item-card_start">
          ...
          <span class="item-card_start_status" v-if="info.send_status == 1">
          ...
          <span class="item-card_start_status" v-else-if="info.send_status == 3">
      ...
      <div class="item-card-bottom__hover">
        ...
        <template>
          <vh-button round type="default" ghost size="mini" @click="openNoticeDialog">
            发送记录
          </vh-button>
        </template>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      switchChangeOpen(value) {
        if (
          this.noticeApp &&
          this.noticeApp.smsBalance &&
          this.noticeApp.smsBalance.sms == 0 &&
          value == 1
        ) {
      ...
      openDialog(type) {
        if (type === 'link') {
          ...
          const isJoinStr =
            ...
            short_url.indexOf('//') != -1;
          this.info.short_url &&
            window.open(`${isJoinStr ? '' : '//'}${this.info.short_url}`, '_blank');
        } else {
