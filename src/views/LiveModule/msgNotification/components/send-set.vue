  ...
  >
    <div v-loading="isLoading">
      ...
      <div class="set-item">
        ...
        <div class="set-item__content">
          ...
          <vh-checkbox-group
            v-model="send_user"
            @change="checkSelect"
            :disabled="senderUserShowLength == 1"
          >
            <template v-for="(item, index) in senderUserOptions">
              <vh-checkbox :label="item.label" :key="`send_${index}`" v-if="item.isShow">
                {{ item.text }}
                <vh-tooltip
                  v-tooltipMove
                  v-if="item.label == 1"
                ...
                >
                  ...
                  <div slot="content" v-else>
                    ...
                    <p>
                      2. 当开启专题统一观看限制、统一报名表单时（免费条件及密码条件无
                      <br />
                      预约提交手机号功能因此无法触发短信）：如果专题下的多个活动开启了
                      <br />
                      短信通知，则将对该活动下设置的短信发送对象+通过专题观看权限的用
                      <br />
                      户发送开播提醒或回放提醒。
                    </p>
                  </div>
      ...
      <div class="set-item import_excel_info" v-if="send_user.includes('3')">
        ...
        <div class="set-item__content">
          <import-excel
            ...
            @uploadKey="uploadKeySet"
            v-if="cardInfo && noticeDetailVo"
            :visible="cardInfo && noticeDetailVo"
      ...
      <div class="set-item send_info">
        ...
        <div class="set-item__content">
          <div class="set-item__content_center">
            <div class="set-item__content_center__ctx">
              {{ cardQueryVo && cardQueryVo.content_str_max ? cardQueryVo.content_str_max : '' }}
              <span @click="openShortLink" class="set-item__content_center__link">
                {{
                  cardQueryVo && cardQueryVo.short_url ? hideString(cardQueryVo.short_url, 20) : ''
                }}
              </span>
            </div>
          </div>
          <p class="set-item__content_bottom">
            <span>
              短信字数：
              <strong>{{ smsCensus.wordage }}</strong>
              （含退订后缀）
            </span>
            <span>
              单条计费 (条)：
              <strong>{{ smsCensus.rowCount }}</strong>
              （70字符为一条）
            </span>
            <span>
              可用余额 (条)：
              <strong>{{ userSmsAmount }}</strong>
            </span>
          </p>
        </div>
      </div>
        ...
        <div class="set-item__content">
          <vh-checkbox-group v-model="send_time" v-if="cardInfo.config_type == 2" size="small">
            <vh-checkbox
              ...
              round
              :class="
                send_time.includes(item.value) && getCheckStatus(item) == 3
                  ? 'old-timer'
                  : send_time.includes(item.value) && getCheckStatus(item) != 3
                  ? 'yes-timer'
                  : ''
          ...
          <span class="set-item__content__default" v-else-if="cardInfo.config_type == 1">
            预约/报名成功后发送
          </span>
          <span class="set-item__content__default" v-else-if="cardInfo.config_type == 3">
            设置默认回放后发送
          </span>
          <span class="set-item__content__default" v-else>——</span>
          <p v-if="[2, 3].includes(cardInfo.config_type)" class="set-item__content__desc">
            {{
              cardInfo.config_type == 2
                ? `注意：若勾选已错过的时间点将不进行发送，当前开播时间：${
                    noticeDetailVo &&
                    noticeDetailVo.webinar_info &&
                    noticeDetailVo.webinar_info.start_time
                      ? noticeDetailVo.webinar_info.start_time
                      : '--'
                  }`
                : '注意：当前活动仅发送一次'
      ...
      <div class="set-dialog__footer">
        <p class="set-dialog__footer_left">
          <span class="set-item__test" @click="openTestDialog">发送测试短信</span>
        </p>
        <vh-button
          type="primary"
          size="medium"
          ...
          borderRadius="50"
          @click="saveInfo"
          :disabled="send_user.includes('3') && btnDisabled"
          v-preventReClick
        >
          确定
        </vh-button>
    ...
    >
      ...
      <div class="dialog-footer">
        ...
        <vh-button
          ...
          v-preventReClick
          @click="sendTest"
          :disabled="!phoneForm.phone || (phoneForm.phone && phoneForm.phone.length != 11)"
        >
          立即发送
        </vh-button>
      </div>
  ...
  export default {
      ...
      senderUserOptions: function () {
        return [
          ...
          },
          {
            ...
            text: '白名单用户',
            isShow:
              this.cardInfo.config_type != 1 &&
              this.noticeApp &&
              this.noticeApp.WEBINAR_PES['white_list'] &&
              this.isSetWhite
          }
        ];
      },
    ...
    methods: {
      ...
      openShortLink() {
        ...
        const isJoinStr =
          short_url.indexOf('https://') != -1 ||
          short_url.indexOf('http://') != -1 ||
          short_url.indexOf('//') != -1;
        this.cardQueryVo.short_url &&
          window.open(`${isJoinStr ? '' : '//'}${this.cardQueryVo.short_url}`, '_blank');
      },
      getCheckStatus(item) {
        if (
          this.noticeDetailVo &&
          this.noticeDetailVo.sms_info &&
          this.noticeDetailVo.sms_info.send_res
        ) {
      ...
      async ajaxSetSave() {
        ...
        this.$fetch('saveSendSet', this.$params(this.saveSetParams))
          .then(res => {
      ...
      formatContentStr(resVo, len) {
        ...
        let subjectByType =
          subject.length + smsSign.length > len
            ...
            : subject;
        let text = resVo.sms_info.content_template
          .replace('${sms_sign}', smsSign)
          .replace('${subject}', subjectByType)
          .replace(new RegExp(resVo.sms_info.short_url, 'g'), '');
        // 格式化列表展示文案
        ...
        return this.$fetch('viewerSetGet', {
          ...
          .then(res => {
            if (res.code == 200 && res.data) {
              this.isSetWhite =
                res.data.verify == 2 ||
                (res.data.parent_verify == 2 && res.data.subject_verify == 1);
            } else {
              this.isSetWhite = '';
            }
          })
  ...
  /deep/.send_time {
    ...
    .vh-checkbox.is-bordered.vh-checkbox--small {
      ...
      &.old-timer {
        ...
        .vh-checkbox__input.is-indeterminate .vh-checkbox__inner {
