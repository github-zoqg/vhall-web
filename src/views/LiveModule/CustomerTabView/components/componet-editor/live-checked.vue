  ...
  <div>
    <div class="vh-chose-active-box menus-checkedBox">
      <div class="vh-chose-active-item" v-if="activeList.length == 0">
        ...
        <div class="vh-chose-active-item__title ellsips">示例活动</div>
        <div class="vh-chose-active-item__info">2020-12-30 12:00:00</div>
      </div>

      <!-- 单个视频 -->
      <div class="vh-chose-active-item" v-for="item in activeList" :key="item.webinar_id">
        <div class="vh-chose-active-item__cover">
          <!-- TODO 选择直播后，反显已选择面板（至少有一个已选中的，后续选择都在此面板） -->
          <img :class="`img_box_bg box_bg_${item.itemMode}`" :src="item.img_url" alt="" />
          <span class="vh-chose-active-item__del" @click.stop="delActiveItem(item.id, item.player)">
            <img src="../images/icon-trash-line-01.png" alt="" />
          </span>
          <div
            class="vh-chose-active-item__cover-status zdy"
            :class="{ smallSize: hasDelayPermission && item.no_delay_webinar == 1 }"
          >
            <span class="liveTag">
              <!-- <label class="live-status" v-if="item.webinar_state == 1">
              <img src="../../../../../common/images/live.gif" alt="">
