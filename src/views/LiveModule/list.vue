            ...
            <div class="bottom">
              ...
              <p class="liveOpera" @click.stop="nullFunc">
                <el-tooltip
                  ...
                  content="开播"
                  placement="top"
                  v-if="item.webinar_state != 4 && item.webinar_type != 5"
                  v-tooltipMove
                >
                ...
                >
                  <i
                    ...
                    @click.prevent.stop="toCopy(item.webinar_id, item.webinar_show_type)"
                  ></i>
                </el-tooltip>
    ...
    methods: {
      ...
      commandMethod(command) {
        if (command === '删除') {
          ...
          const { href } = this.$router.resolve({
            path: `${command}/${this.webinarInfo.webinar_id}`,
        ...
        const { href } = this.$router.resolve({
          path:
            item.webinar_state == 4
              ? `/live/recordplayback/${item.webinar_id}`
              : `/live/playback/${item.webinar_id}`
        });
        window.open(href, '_blank');
      },
      goIsLive(item) {
