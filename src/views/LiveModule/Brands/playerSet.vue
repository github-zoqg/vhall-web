    ...
    <div v-if="(hasDelayPremission && showDelay && webinar_type != 6) || (webinar_type == 6 && showGroupLive)" class="delay-mask">
      <div class="tip">
        ...
        <div class="delay-content">
          {{webinar_type == 6 ? '当前直播活动为分组直播，暂不支持设置跑马灯、水印、弹幕功能。以上功能仅生效于回放状态。' : '当前模式为无延迟模式，暂不支持设置跑马灯、水印、弹幕功能。以上功能仅生效于回放状态。'}}
        </div>
        <div class="btn" @click.stop="closeDialog">
          我知道了
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      videoParam: {
        paas_record_id: process.env.VUE_APP_NODE_ENV === 'production' ? 'd57f42f4': '52b2be4a'
      },
      marqueeOption: {
