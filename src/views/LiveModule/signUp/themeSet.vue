<template>
  ...
  >
    ...
    <div :class="['tabs', colorIndex]">
      <div :class="{ active: tabs == 'title' }" @click="tabs = 'title'">
        <pre>{{ title1 || '活动报名' }}</pre>
      </div>
      <div :class="{ active: tabs == 'valite' }" @click="tabs = 'valite'">
        <pre>{{ title2 || '我已报名' }}</pre>
      </div>
    </div>
    ...
    <span slot="footer" class="dialog-footer">
      <el-button type="primary" v-preventReClick @click.prevent.stop="save" round size="medium">
        保存
      </el-button>
    </span>
  </VhallDialog>
