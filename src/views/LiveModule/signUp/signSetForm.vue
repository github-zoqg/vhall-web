<template>
  <div id="settingBox" class="settingBox clearFix">
    <ul
      :class="['options', menuBarFixed]"
      v-show="tabType === 'form' && rightComponent != 'signUpForm'"
    >
  ...
  export default {
    ...
    methods: {
      ...
      getQuestionList() {
        ...
        this.$fetch('regQListGet', this.setParamsIdByRoute({}))
          .then(res => {
            // 按照 order_num 从小到大排序
      ...
      addFiled(info, opts) {
        ...
        if (opts) {
          ...
          if (
            ...
            info.name === 'duty' ||
            (info.type === 'select' && opts.items && opts.items.length)
          ) {
            ...
            const optsOptions1 =
              optsOptions.length === 2 ? JSON.parse(optsOptions[1].options) : false;
          ...
          .then(res => {
            if (res.code === 200) {
              ...
              if (
                ...
                (info.name === 'duty' || info.type === 'select') &&
                res.data.items &&
                res.data.items.length
              ) {
                res.data.items.forEach((item, index) => {
              ...
              this.$nextTick(() => {
                ...
                document.documentElement.scrollTo(
                  0,
                  this.$refs.fieldSet.$refs[res.data.id][0].offsetTop - 70
                );
              });
