    ...
    <div class="weixinWork" v-for="(item, index) in languageList" :key="index">
      ...
      <div class="input-card">
        <div class="input-item">
          <span>标准观看页</span>
          <el-input
            :value="handleLiveContent(item.language_type)"
            readonly
            ...
            <template slot="suffix">
              <el-tooltip class="item" effect="dark" content="复制" placement="top" v-tooltipMove>
                <i
                  ...
                  :title="'复制'"
                  @click="cope(handleLiveContent(item.language_type), 1)"
                ></i>
              </el-tooltip>
            </template>
          </el-input>
          <el-popover popper-class="lang-scan" placement="bottom" trigger="hover">
            <div class="invitation-code">
              ...
              <div class="download" @click="download(item.language_type, item.subject)">
                下载二维码
              </div>
            </div>
            <el-button round size="medium" class="transparent-btn" slot="reference">
              扫码观看
            </el-button>
          </el-popover>
        </div>
            ...
            <template slot="suffix">
              <el-tooltip class="item" effect="dark" content="复制" placement="top" v-tooltipMove>
                <i
                  class="iconfont-v3 saasicon_copy"
                  :title="'复制'"
                  @click="cope(handleCompletion(item.language_type), 2)"
                ></i>
              </el-tooltip>
            </template>
          </el-input>
            ...
            <template slot="suffix">
              <el-tooltip class="item" effect="dark" content="复制" placement="top" v-tooltipMove>
                <i
                  class="iconfont-v3 saasicon_copy"
                  :title="'复制'"
                  @click="cope(handlerVideo(item.language_type), 2)"
                ></i>
              </el-tooltip>
            </template>
          </el-input>
  ...
  export default {
    ...
    methods: {
      ...
      handlerVideo(lang = 1) {
        ...
        const url = `<iframe border="0" allow="display-capture" allowfullscreen="true" src="${env}/lives/embedclient/watch/${il_id}?embed=video&lang=${
          lang == 2 ? 2 : 1
      ...
      handleCompletion(lang = 1) {
        const env = process.env.VUE_APP_WAP_WATCH;
        const il_id = this.$route.params.str;
        const url = `<iframe allow="camera *;microphone *; display-capture" allowfullscreen="true" border="0" src="${env}/lives/embedclient/watch/${il_id}?lang=${
          lang == 2 ? 2 : 1
        }" width="800" height="600"></iframe>`;
        // this.completion = url
        return url;
