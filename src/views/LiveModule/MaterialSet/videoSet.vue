<template>
  <div class="video-wrap">
    ...
    <div class="head-operat" v-show="total || isSearch">
      <el-button size="medium" type="primary" round class="length104 head-btn set-upload">
        上传
        <input
          ref="uploads"
          class="set-input"
      ...
      <el-button size="white-medium" round class="transparent-btn" @click="choiceVideo">
        资料库
      </el-button>
      <el-button
        size="medium"
    ...
    <div class="video-list" v-if="total || isSearch">
      ...
      >
        <el-table-column type="selection" width="52" align="left" :selectable="checkSelectable" />
            ...
            >
              <div class="videoName custom-tooltip-content">
                <i
                  ...
                  v-if="scope.row.file_type == '.mp3' || scope.row.file_type == '.mav'"
                ></i>
        ...
        <el-table-column width="200" prop="created_at" label="上传时间"></el-table-column>
        <el-table-column width="200" prop="duration" label="时长"></el-table-column>
        <el-table-column prop="transcode_status_text" width="200" label="进度">
          <template slot-scope="scope">
            <div>
        ...
        <el-table-column width="150" label="操作">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="del(scope.row)" v-if="scope.row.transcode_status >= 0">
              删除
            </el-button>
          </template>
        </el-table-column>
      ...
      >
        <div class="edit-contianer">
          ...
          <div class="dialog-footer">
            ...
            <el-button size="medium" round class="length104" @click="editShowDialog = false">
              取消
            </el-button>
          </div>
        </div>
      ...
      <div class="async__body">
        ...
        <div class="async__footer">
          <el-button
            ...
            size="medium"
            v-preventReClick
            @click="comfirmMaterialVideo"
            round
          >
            确 定
          </el-button>
          <el-button size="medium" v-preventReClick @click="cancelMaterialVideo" round>
            取 消
          </el-button>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$EventBus.$on('waiting_sign_trans_code', res => {
        // 转码状态
    ...
    methods: {
      ...
      tirggerFile(event) {
        ...
        if (!videoType) {
          this.$message({
            message:
              this.webinarType == 1 ? '音频直播仅支持上传音频文件' : '您上传的文件格式不正确',
            showClose: true,
            // duration: 0,
            type: 'error',
      ...
      getList(obj) {
        this.$fetch('waitingVideoList', this.$params(obj))
          .then(res => {
            if (res.code == 200) {
      ...
      confirmDelete(id, index) {
        this.$confirm('删除后将会影响视频的演示和观看，确定删除？', '提示', {
          ...
          .then(() => {
            this.$fetch('waitingVideoDelete', { ids: id })
              .then(res => {
                if (res.code == 200) {
                  // this.$vhall_paas_port({
    ...
    beforeDestroy() {
      ...
      //   this.UploadSDK = null;
      // }
      this.$EventBus.$off('waiting_sign_trans_code');
    }
  };
</script>
<style lang="less">
