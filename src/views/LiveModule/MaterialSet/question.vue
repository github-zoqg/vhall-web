<template>
  <div class="question-wrap">
    <pageTitle pageTitle="问卷">
      <div slot="content">
        修改已收集数据的问卷，按照修改后的问卷重新统计，为确保问卷统
        <br />
    ...
    <div class="head-operat" v-show="total || isSearch">
      ...
      <el-button round @click="dataBase" class="transparent-btn" size="white-medium">
        资料库
      </el-button>
      <el-button
        round
  ...
  export default {
    ...
    methods: {
      ...
      edit(that, { rows }) {
        if (rows.publish) {
          that
            .$confirm('当前问卷已被推送，修改将影响之前收集的数据，确认修改?', '提示', {
      ...
      deleteConfirm(id, index) {
        this.$confirm('删除后，此问卷将无法使用，确认删除?', '提示', {
          ...
          .then(() => {
            this.$fetch('deleteLiveQuestion', { survey_ids: id, webinar_id: this.webinarId })
              .then(res => {
                this.$vhall_paas_port({
      ...
      addQuestion() {
        this.$router.push({
          path: '/live/addQuestion',
          query: { webinarId: this.webinarId, roomId: this.$route.query.roomId, type: 2 }
        });
      },
      dataBase() {
