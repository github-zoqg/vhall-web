      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="商品链接" prop="url">
          <VhallInput
            ...
            v-clearEmoij
            placeholder="请输入以http://或https://开头的商品链接"
            autocomplete="off"
            show-word-limit
            :maxlength="200"
          ></VhallInput>
        ...
        <el-form-item label="淘口令">
          <VhallInput
            v-model.trim="form.tao_password"
            v-clearEmoij
            placeholder="请输入淘口令"
            autocomplete="off"
            show-word-limit
        ...
        <el-form-item label="店铺链接" prop="shop_url">
          <VhallInput
            v-model.trim="form.shop_url"
            v-clearEmoij
            placeholder="请输入以http://或https://开头的店铺链接"
            autocomplete="off"
            show-word-limit
            :maxlength="200"
          ></VhallInput>
        ...
        <el-form-item>
          <el-button type="primary" round class="length152" v-preventReClick @click="onSubmit">
            保存
          </el-button>
        </el-form-item>
      </el-form>
  ...
  export default {
        ...
        rules: {
          order_num: [
            { required: true, message: '请输入商品排序', trigger: 'blur' },
            {
              ...
              min: 0,
              max: 9999,
              message: '商品排序必须为0-9999数字值',
              trigger: 'blur'
            }
          ],
          name: [{ required: true, validator: nameValidate, trigger: 'blur' }],
          img_id: [{ required: true, validator: imgValidate, trigger: 'change' }],
          description: [{ required: true, message: '请输入商品描述', trigger: 'blur' }],
          price: [{ required: true, validator: priceValidate, trigger: 'blur' }],
          discount_price: [{ required: false, validator: priceCountValidate, trigger: 'blur' }],
          url: [{ required: true, validator: linkValidate, trigger: 'blur' }],
          shop_url: [{ required: false, validator: shopValidate, trigger: 'blur' }],
    ...
    watch: {
      ...
      'form.discount_price'(val) {
        this.form.discount_price = val.match(/\d+(\.\d{0,2})?/)
          ? val.match(/\d+(\.\d{0,2})?/)[0]
          : '';
      }
    },
    methods: {
      ...
      onSubmit() {
        this.$refs['form'].validate(valid => {
          if (valid) {
            this.defaultCover = this.fileList
              .filter(item => item.cover)
              .map(item => item.img_id)
              .join(',');
