<template>
  <div class="question-wrap">
    ...
    <div class="head-operat" v-show="total || isSearch">
      <el-button type="primary" size="medium" round @click="addProduct" v-preventReClick>
        创建商品
      </el-button>
      <el-button
        round
        ...
        v-preventReClick
        :disabled="!checkedGoodsId.length"
      >
        批量删除
      </el-button>
      <div class="inputKey">
    ...
    <div class="empty" v-show="!total && !isSearch">
      <noData :nullType="'nullData'" :text="'您还没有商品，快来创建吧！'">
        <el-button type="primary" round class="length152" @click="addProduct" v-preventReClick>
          创建
        </el-button>
      </noData>
    </div>
    ...
    methods: {
      ...
      addCover() {
        this.tableData.length &&
          this.tableData.forEach(item => {
            item.img =
              item.img_list.find(item => item.is_cover) > -1
      ...
      delConfirm(id, index) {
        this.$confirm('确定要删除该商品吗？', '提示', {
          ...
          .then(() => {
            this.$fetch('goodsBatchDel', { webinar_id: this.$route.params.str, goods_ids: id })
              .then(res => {
                if (res.code == 200) {
                  this.$vhall_paas_port({
      ...
      // 获取在线商品列表
      getSaleGoodsList() {
        this.$fetch('goodsList', { webinar_id: this.$route.params.str, pos: 0, limit: 1 })
          .then(res => {
            if (res.code == 200) {
      ...
      changeItemInput(data) {
        ...
        if (
          ...
          (data.order_num > 0 && data.order_num < 1) ||
        ...
        this.$fetch('setGoodOrder', params)
          .then(res => {
            this.$vhall_paas_port({
