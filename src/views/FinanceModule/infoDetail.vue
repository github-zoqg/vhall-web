<template>
  <div class="account-income">
    <pageTitle pageTitle="订单明细">
      <div slot="content">
        购买明细指用户从后台主动下单购买相关功能的订单；开通明细指通过工作人
        <br />
        员给您开通的相关功能订单
      </div>
    </pageTitle>
      ...
      <el-tabs v-model="activeIndex" @tab-click="handleClick">
        ...
        <div class="interact-detail">
          ...
          >
            ...
            <el-table-column label="交易时间" width="180">
              <template slot-scope="scope">
                <el-tooltip
                  ...
                  :content="scope.row.create_time == '' ? '- -' : scope.row.create_time"
                >
            ...
            <el-table-column prop="type" label="订单类型" width="150"></el-table-column>
            <el-table-column
              prop="amount"
              v-if="activeIndex == 1"
              label="交易金额"
              width="120"
            ></el-table-column>
            <el-table-column prop="content" label="购买内容" width="120"></el-table-column>
            <el-table-column label="订单状态" v-if="activeIndex == 1" width="120">
              <template slot-scope="scope">
            ...
            <el-table-column v-if="activeIndex == 2" label="订单状态" width="120">
              <template slot-scope="scope">
                <span class="buyStatus">
                  <i
                    :class="
            ...
            <el-table-column v-if="activeIndex == 2" label="来源" width="120">
              <template slot-scope="scope">
                <span>{{ scope.row.source | filterSource }}</span>
              </template>
            </el-table-column>
            <el-table-column label="启用日期" width="180">
              <template slot-scope="scope">
                <el-tooltip
                  placement="top"
                  :disabled="!isTextOverflow"
                  :content="scope.row.start_time == '' ? '- -' : scope.row.start_time"
                >
                  <p class="custom-tooltip-content">
            ...
            <el-table-column label="失效日期" width="180">
              <template slot-scope="scope">
                <el-tooltip
                  placement="top"
                  :disabled="!isTextOverflow"
                  :content="scope.row.end_time == '' ? '- -' : scope.row.end_time"
                >
                  <p class="custom-tooltip-content">
            ...
            >
              <template slot-scope="scope">
                ...
                <el-button type="text" @click="pay(scope.row)" v-if="!scope.row.status">
                  立即支付
                </el-button>
              </template>
            </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getSearchList() {
        ...
        for (let i in formParams) {
          ...
          if (i === 'orderType') {
            ...
            let typeDetail = [
              100730, 100731, 100732, 100733, 100734, 100735, 100736, 100737, 100726, 100727,
              100728, 100729
            ];
      ...
      getDetailList(params) {
        ...
        this.$fetch(url, this.$params(obj))
          .then(res => {
            ...
            if (res.data.total > 0) {
              tableList.map(item => {
                if (this.activeIndex == 1) {
                  item.statusText =
                    item.status == 1 ? '成功' : item.status == -1 ? '失败' : '待支付';
                  item.type = this.culesType(item.type);
                } else {
                  item.statusText =
                    item.status == 1 ? '生效中' : item.status == -1 ? '已失效' : '待生效';
      ...
      exportAccount() {
        ...
        this.$fetch(url, this.$params(this.params))
          .then(res => {
            this.$vhall_paas_port({
