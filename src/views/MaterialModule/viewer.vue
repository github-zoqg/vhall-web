  ...
  <div class="audienceBox">
    ...
    <div class="div__main" v-if="groupList.length > 0">
      <div class="table__container">
        <!-- 操作栏 -->
        <div class="operaBox">
          <el-button type="primary" round @click.prevent.stop="viewerDialogAdd" size="medium">
            新增观众
          </el-button>
          ...
          <el-button round size="medium" v-if="downloadUrl" @click="downloadTemplate">
            下载模版
          </el-button>
          <el-button
            round
          ...
          <div class="searchBox">
            <VhallInput
              ...
              @clear="search(1)"
            >
              <i class="el-icon-search el-input__icon" slot="prefix" @click="search(1)"></i>
            </VhallInput>
          </div>
        </div>
        <!-- 列表 -->
    ...
    <div v-else>
      <null-page :nullType="'nullData'" :text="'您还没有观众分组，快来创建吧！'">
        <el-button type="primary" round @click="addGroupDialogShow(null)" v-preventReClick>
          添加分组
        </el-button>
      </null-page>
    </div>
    ...
    >
      <el-form :model="groupForm" ref="groupForm" :rules="groupFormRules" @submit.native.prevent>
        <el-form-item prop="subject">
          <VhallInput
            v-model.trim="groupForm.subject"
            auto-complete="off"
            placeholder="请输入分组名称"
            :maxlength="15"
            :minlength="1"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="postGroupSend('groupForm')" round size="medium">
          确定
        </el-button>
    ...
    <VhallDialog :title="viewerDialog.title" :visible.sync="viewerDialog.visible" width="484px">
      ...
      >
        ...
        <el-form-item label="工号" prop="job_number">
          <VhallInput
            v-model.trim="viewerForm.job_number"
            auto-complete="off"
            placeholder="请输入工号（最多50个字符）"
            :maxlength="50"
          />
        </el-form-item>
        <el-form-item label="小组" prop="group_order_id">
          <VhallInput
            v-model.trim.number="viewerForm.group_order_id"
            v-clearEmoij
            auto-complete="off"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          ...
          :disabled="
            !viewerForm.name ||
            (!viewerForm.email && !viewerForm.phone && !viewerForm.job_number && !viewerForm.other)
          "
        >
          确定
        </el-button>
    ...
    <!-- 预设小组-->
    <default-group ref="defaultGroup" @search="search(1)" :groupId="query.group_id"></default-group>
  </div>
</template>
  ...
  export default {
      ...
      return {
        ...
        tableColumn: [
          ...
          },
          {
            ...
            showHeader: true,
            headerTooltip:
              '此字段只适用于分组直播中的分组预导入功能，用来将观众提前分配到小组中，非必填字段'
          },
          {
            label: '其它',
            key: 'other',
        ...
        viewerFormRules: {
          ...
          group_order_id: [{ required: false, validator: validGroup, trigger: 'blur' }],
          other: [
    ...
    methods: {
      ...
      audienceGet() {
        ...
        this.$fetch('audienceGet', this.$params(params))
          .then(res => {
            res && res.code === 200 && res.data && res.data.list
        ...
        debounce(() => {
          this.$refs.groupForm.validate(valid => {
            if (valid) {
              ...
              this.$fetch(
                this.groupDialog.type === 'add' ? 'postGroupAdd' : 'postGroupEdit',
                this.$params(params)
              )
                .then(res => {
                  ...
                  this.$message({
                    message: `${
                      this.groupDialog.type === 'add' ? '添加分组' : '重命名分组'
                    }操作成功`,
                    showClose: true,
                    // duration: 0,
                    type: 'success',
                ...
                .catch(res => {
                  ...
                  this.$message({
                    message:
                      res.msg ||
                      `${this.groupDialog.type === 'add' ? '添加分组' : '重命名分组'}操作失败`,
                    showClose: true,
                    // duration: 0,
                    type: 'error',
                    customClass: 'zdy-info-box'
      ...
      postGroupDel(item) {
        this.$confirm('确定要删除当前分组？', '删除组', {
          ...
          .then(() => {
            ...
            this.$fetch('postGroupDel', this.$params(params))
              .then(res => {
                this.$vhall_paas_port({
                  k: 100548,
                  data: {
      ...
      viewerList(row) {
        ...
        this.$fetch('viewerList', this.$params(this.query))
          .then(res => {
            res && res.code === 200 && res.data && res.data.total > 0
          ...
          if (valid) {
            ...
            let params = Object.assign(
              this.viewerDialog.type === 'add'
                ...
                : { id: this.viewerDialog.row.id, group_id: this.query.group_id },
              this.viewerForm
            );
            this.$fetch(this.viewerDialog.type === 'add' ? 'viewerAdd' : 'viewerEdit', params)
              .then(res => {
                this.$vhall_paas_port({
                ...
                this.$message({
                  message: `${
                    this.viewerDialog.type === 'add' ? '添加观众' : '观众信息修改'
                  }操作成功`,
                  showClose: true,
                  // duration: 0,
                  type: 'success',
              ...
              .catch(res => {
                ...
                this.$message({
                  message:
                    res.msg ||
                    `${this.viewerDialog.type === 'add' ? '添加观众' : '观众信息修改'}操作失败`,
                  showClose: true,
                  // duration: 0,
                  type: 'error',
                  customClass: 'zdy-info-box'
