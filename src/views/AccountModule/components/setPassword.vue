     ...
     <div v-if="step === 1">
       <div class="content">
         <el-form :model="levelForm" ref="levelForm" :rules="levelFormRules" label-width="85px">
           <p class="warning-info">为了保证您的账号安全，修改密码请先验证绑定的手机号，验证成功后进行下一步操作</p>
           ...
           <el-form-item label="验证码" prop="code" class="no-margin">
             <el-input v-model.trim="levelForm.code" auto-complete="off" placeholder="请输入验证码">
               <el-button :disabled="isVerifyPhone" v-if="hadSend" @click="time"  class="no-border" size="mini" slot="append">发送验证码</el-button>
               <el-button v-if="!hadSend" :disabled="isClick" @click="time"  class="no-border" size="mini" slot="append">{{getCode}}</el-button>
             </el-input>
           </el-form-item>
       ...
       <div class="content">
         <el-form :model="form" ref="form" :rules="formRules" label-width="85px">
           <el-form-item label="原密码" prop="old_password" v-if="vo.has_password > 0">
             <el-input v-model.trim="form.old_password" auto-complete="off" placeholder="请输入原密码"/>
           </el-form-item>
           <el-form-item label="新密码" prop="password">
             <el-input v-model.trim="form.password" auto-complete="off" placeholder="请输入新密码"/>
           </el-form-item>
           <el-form-item label="再次输入" prop="confirm_password">
             <el-input v-model.trim="form.confirm_password" auto-complete="off" placeholder="请再一次输入新密码"/>
           </el-form-item>
         </el-form>
       </div>
     </div>
...
export default {
  ...
  methods: {
    ...
    time: function () {
      ...
      if (this.numCode == 0) {
        ...
        this.numCode--;
        this.timeoutA = setTimeout(function () { self.time(); }, 1000);
      }
    },
    //获取短信验证码
    sendVerifyMsg() {
    ...
    cancelHandle() {
      this.step === 2 ? this.step = 1 : this.visible = false;
    },
    validatePhone() {},
    validatePwd() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          let params = this.form.scene_id === 1 ? {
 ...
 /* watch: {
    'form.password': function () {
      this.isReset = !(this.checkPassword(this.form.password) && this.form.password === this.form.confirm_password
