  ...
  <div class="editBox">
    ...
    <el-form :model="formData" ref="ruleForm" :rules="rules" v-loading="loading" label-width="80px">
      ...
      <el-form-item label="专题封面">
        ...
        <div class="image_cropper">
          <div class="image_cropper_item">
            <span>模糊程度</span>
            <vh-slider
              v-model="cropperImage.blurryDegree"
          ...
          <div class="image_cropper_item">
            <span>背景亮度</span>
            <vh-slider
              v-model="cropperImage.lightDegree"
              :disabled="!formData.imageUrl"
              :max="20"
              style="width: 480px"
            ></vh-slider>
      ...
      <el-form-item label="">
        ...
        <el-button class="length152" @click="resetForm('ruleForm')" v-preventReClick round>
          取消
        </el-button>
      </el-form-item>
    </el-form>
  ...
  export default {
    ...
    computed: {
      domain_url() {
        ...
        }?x-oss-process=image/crop,x_${this.cropperImage.backgroundSize.x.toFixed()},y_${this.cropperImage.backgroundSize.y.toFixed()},w_${this.cropperImage.backgroundSize.width.toFixed()},h_${this.cropperImage.backgroundSize.height.toFixed()}${
          this.cropperImage.blurryDegree > 0
            ? `,/blur,r_10,s_${this.cropperImage.blurryDegree * 2}`
            : ''
        },/bright,${(this.cropperImage.lightDegree - 10) * 5}&mode=${
          this.cropperImage.imageCropMode
        }`;
      },
      reservationDesc() {
