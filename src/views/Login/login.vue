<template>
  <div class="wapper wapperClass">
    <div class="left fl">
      <div>
        <img
          src="../../common/images/login/login-logo.png"
          alt=""
          class="logo"
        />
        <div class="content">
          <p>
            针对不同类型、不同行业企业业务和使用场景，<br />专注于为企业提供通用型场景化直播解决方案和服务
          </p>
          <img
            src="../../common/images/login/login-advert.png"
            alt=""
            class="bg"
          />
        </div>
    ...
    <div class="right fr">
      ...
      <div class="login-box" v-if="$route.path === '/login'">
        <h3>欢迎登录微吼直播</h3>
        <p class="tab">
          <span @click="changeLogin('1')" :class="isActive == 1 ? 'active' : ''"
            >账号登录</span
          >
          <em>|</em>
          <span @click="changeLogin('2')" :class="isActive == 2 ? 'active' : ''"
            >手机登录</span
          >
        </p>
        <!-- 账号登录 -->
        <div class="user-wapper form-items" v-if="isActive === 1">
          <el-form ref="loginForm" :model="loginForm" :rules="loginRules">
            <el-form-item prop="account" class="loginUsername">
              <p
                class="itemLable"
                :class="{ active: isLoginAccountFocus || loginForm.account }"
              >
                用户名 / 邮箱 / 手机号
              </p>
              <VhallInput
            ...
            <el-form-item v-show="isLogin" id="captcha-box">
              <div id="loginCaptcha" class="captcha">
                <VhallInput auto-complete="off" v-model.trim="loginForm.text">
                </VhallInput>
              </div>
            </el-form-item>
            <el-form-item prop="password" class="loginPassword">
              <p
                class="itemLable"
                :class="{ active: isLoginPasswordFocus || loginForm.account }"
              >
                密码
              </p>
              <VhallInput
            ...
            <div class="login-other">
              其他登录方式<span @click="openOther"
                >&nbsp;&nbsp;展开
                <i
                  :class="
                    isOpenOther ? 'el-icon-arrow-down' : 'el-icon-arrow-up'
                  "
                ></i
              ></span>
              <div :class="['other-img', !isOpenOther ? 'noVisible' : '']">
                ...
                <img
                  ...
                  alt=""
                  @click="
                    thirdLogin('/v3/commons/auth/weixin?source=pc&jump_url=')
                  "
                />
          ...
          <el-form ref="dynamicForm" :model="dynamicForm" :rules="loginRules">
            <el-form-item prop="phoneNumber" class="loginUsername">
              <p
                class="itemLable"
                :class="{
                  active: isLoginPhoneFocus || dynamicForm.phoneNumber,
                }"
              >
                手机号
              </p>
              ...
              <p class="errorText" v-if="isRegister">
                <i class="el-icon-error"></i>该手机号未注册，请先注册
              </p>
            </el-form-item>
            <!-- 这个 id 有两个，只为覆盖样式 -->
            <el-form-item id="captcha-box">
              <div id="loginCaptcha" class="captcha">
                <VhallInput auto-complete="off" v-model.trim="dynamicForm.text">
                </VhallInput>
              </div>
            </el-form-item>
            <el-form-item prop="dynamic_code">
              <div class="code">
                <p
                  class="itemLable"
                  :class="{
                    active: isLoginPasswordFocus1 || dynamicForm.dynamic_code,
                  }"
                >
                  动态密码
                </p>
                ...
                >
                  <template slot="append">
                    <span
                      ...
                      :class="
                        showCaptcha && isValidaLoginPhone ? 'isLoginActive' : ''
                      "
                      >{{
              ...
              <p class="errorText" v-show="errorMsgShow">
                <i class="el-icon-error"></i>{{ errorMsgShow }}
              </p>
            </el-form-item>
            <div class="login-btn">
              <el-button
        ...
        <div class="form-items">
          ...
          >
            <el-form-item prop="phone" class="phone-box">
              <p
                class="itemLable"
                :class="{ active: isPhoneFocus || registerForm.phone }"
              >
                手机号
              </p>
              <el-input
            ...
            <el-form-item id="captcha-box">
              <div id="registerCaptcha" class="captcha">
                <el-input auto-complete="off" v-model.trim="registerForm.text">
                </el-input>
              </div>
            ...
            <el-form-item prop="code" class="code">
              <div class="code">
                <p
                  class="itemLable"
                  :class="{ active: isCodeFocus || registerForm.code }"
                >
                  动态密码
                </p>
                <VhallInput
                ...
                >
                  <template slot="append">
                    <span
                      ...
                      :class="
                        showCaptcha && isValidaregisterPhone
                          ? 'isLoginActive'
                          : ''
                      "
                      >{{
            ...
            <el-form-item prop="password" class="password">
              <p
                class="itemLable"
                :class="{ active: isPasswordFocus || registerForm.password }"
              >
                密码
              </p>
              <pwdinput
            ...
            <el-form-item class="auto-login register-checked">
              <el-checkbox v-model="checked"
                ...
                ></el-checkbox
              >
              <span class="toLogin" @click="$router.push({ path: '/login' })"
                >去登录</span
              >
            </el-form-item>
          </el-form>
  ...
  methods: {
    ...
    checkedAccount() {
      let account =
        this.isActive == 1
          ...
          : this.dynamicForm.phoneNumber
      this.$fetch(
        'loginCheck',
        { account: account, channel: 'B' },
        { token: '' }
      )
        .then((res) => {
    ...
    callCaptcha() {
      ...
      initNECaptcha({
        ...
        element: `#${
          this.$route.path === '/login' ? 'loginCaptcha' : 'registerCaptcha'
        }`,
        mode: 'float',
        onReady(instance) {
