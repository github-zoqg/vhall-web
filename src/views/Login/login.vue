...
*/
    ...
    var validateLoginPhone = (rule, value, callback) => {
      ...
      if (value === '') {
        ...
        if (!(/^1[0-9]{10}$/.test(value))) {
          ...
          this.$fetch('loginCheck', {account: value, channel: 'B'}, {token: ''}).then(res => {
  ...
  methods: {
    ...
    checkedAccount() {
      ...
      this.$fetch('loginCheck', {account: account, channel: 'B'}, {token: ''}).then(res => {
    ...
    checkPhone() {
      if (this.checkMobile(this.registerForm.phone)) {
         this.$fetch('loginCheck', {account: this.registerForm.phone, channel: 'B'}, {token: ''}).then(res => {
          if (res.data.account_exist) {
            // 清除密码验证
            try {
